--tạo funtion trả về so luong san pham ban duoc
CREATE FUNCTION Tong_SP(@MaSP char(4))
RETURNS INT--Return phải có s 
AS
BEGIN
declare @TongSP int;
select @TongSP = sum(CTHD.SL) from CTHD where MASP = @MaSP;
return @TongSP;

END
drop function Tong_SP
select dbo.Tong_SP('BB01')
-- tra ve bang cac san pham xuat xu tu Viet Nam!
CREATE FUNCTION Ham_Bang(@QuocGia varchar(40))
returns table
as
return select * from SANPHAM where NUOCSX = @QuocGia;

 select * from dbo.Ham_Bang('Viet Nam') 


 --- Co so du lieu can add
