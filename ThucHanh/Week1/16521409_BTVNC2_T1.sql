--Bai 1
--Co so du lieu add o phia duoi
-- TINH TONG DOANH THU CUA TAT CAC CAC SAN PHAM
SELECT SUM(HOADON.TRIGIA) AS TONGDOANHTHU FROM HOADON
--HIEN THI CAC SP BAN DUOC TRONG THANG

DECLARE @MONTH INT
SET @MONTH = 11;
SELECT DISTINCT SANPHAM.MASP, SANPHAM.TENSP FROM SANPHAM, CTHD 
WHERE SANPHAM.MASP = CTHD.MASP AND 
		CTHD.SOHD = (SELECT HOADON.SOHD FROM HOADON 
						WHERE MONTH(HOADON.NGHD) = @MONTH
						AND HOADON.SOHD = CTHD.SOHD)

--xay dung ham them san pham
CREATE PROCEDURE INSERT_SANPHAM
AS
INSERT INTO SANPHAM VALUES('SP01', 'Giay Note', 'tep', 'Viet Nam', '10000')
exec INSERT_SANPHAM
SELECT * FROM SANPHAM where MASP = 'SP01'	

--bai 2: co ket qua cho tham so dau ra
--Tinh tong doanh thu cua mot san pham
drop proc Tong_DoanhThu

create procedure Tong_DoanhThu(@MaSp char(4), @DoanhThu int out)
as
begin
set @DoanhThu = (select sum(HOADON.TRIGIA) as DoanhThu FROM HOADON, CTHD
WHERE HOADON.SOHD = CTHD.SOHD AND CTHD.MASP = @MaSp)
print @DoanhThu
end
...
EXEC Tong_DoanhThu 'BB01', @DoanhThu out
begin
print @DoanhThu -- tong doanh thu cua san pham BB01
END
